# i'm just gonna copy paste from the orpo script, and then edit this acccording to grpo
...
from transformers import AutoTokenizer, TrainerCallback, get_scheduler
from unsloth.chat_templates import get_chat_template
# from trl import ORPOConfig, ORPOTrainer
from trl import GRPOConfig, GRPOTrainer
# following this script, but this is for vision-lm tho -> https://github.com/huggingface/trl/blob/main/examples/scripts/grpo_vlm.py
# and this is the official doc:
# - grpotrainer : https://huggingface.co/docs/trl/main/en/grpo_trainer
# - grpoconfig : same page, just below
...
class PRE:
    ...
    def evaluate(self, prompt, resp, correct_answer=None):
        try:
            ...
            base = 1.0/(1.0 + perpl/100.0)
            length_bonus = min(0.2, len(resp.split()) / 50.0)
...
class RCF:
    ...
    def train_with_rt(self, ds, rt="hard", num_epochs=NUM_EPOCHS):
        ...
        # this scripts is also nice: https://github.com/huggingface/trl/blob/main/trl/trainer/grpo_config.py#L22
        ...
        optimizer = torch.optim.AdamW(self.model.parameters(), lr=grpo_config.learning_rate)
        trainer = GRPOTrainer(
            model=self.model,
            args=grpo_config,
            ...
            optimizers=(
                ...
                get_scheduler(
                    "linear",
                    optimizer=optimizer,
                    num_warmup_steps=grpo_config.warmup_steps,
                    num_training_steps=grpo_config.num_training_steps,
                ),
            ),
